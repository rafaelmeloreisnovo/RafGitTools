...
5. Treat third-party work as third-party. Porting, translation to another language, cosmetic edits, build changes, renaming, reformatting, optimization, maintenance, or large mechanical rewrites do not establish independent authorship.
...
10. For a known upstream/fork where no separately authorial component needs recognition, default to upstream ownership and attribution. Do not spend effort trying to convert integrated improvements into an authorship claim.
11. Deep authorship-boundary review is currently prioritized for `llamaRafaelia`, `termux-app-rafacodephi`/RafCodePhi components, `BLAKE3` separated authorial area, and `UserLAnd`/`UserLAnd2` separated authorial areas. Upstream remains upstream in all four; only independently evidenced separated components may receive an authorial classification.
12. For `linuxkernel`, `nanoGPT`, `Shizuku`, `OpenSSL`, `gradle`, `qemu_rafaelia`, `florisboard`, and `LuaJIT`, default to upstream/third-party attribution for the upstream body. Correct LICENSE/README/NOTICE/SPDX inconsistencies as required; do not seek ownership credit for integrated maintenance or improvements unless a later human decision explicitly opens a separately evidenced component review.
13. When an obvious, low-risk compliance defect is found and the applicable upstream obligation is already verified, correct it on the audit/work branch immediately and record the evidence. Do not postpone a clear README attribution/link, NOTICE/index metadata, or proven SPDX/header correction merely to build a larger batch.
14. License changes, relicensing, copyright/authorship changes, material removal for licensing reasons, dual licensing, publication, merge or release require human review. Routine attribution/index/link fixes may be prepared on an audit branch when the applicable obligation is already verified.
