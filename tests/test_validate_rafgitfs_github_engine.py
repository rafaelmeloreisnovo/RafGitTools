...
class RafGitFsGithubEngineGateTest(unittest.TestCase):
    ...
    def test_mutation_endpoint_is_rejected(self) -> None:
        with self.mutated_root(MODULE.FILES[0], lambda value: value.replace(
            "interface RafGitFsGithubApiService {",
            'interface RafGitFsGithubApiService {\n    @retrofit2.http.POST("repos/x/y")\n    suspend fun mutate(): Unit',
        )) as root:
            with self.assertRaises(MODULE.ValidationError): MODULE.validate(root)

    def test_missing_tree_truncation_guard_is_rejected(self) -> None:
        with self.mutated_root(MODULE.FILES[5], lambda value: value.replace(
            "GITHUB_TREE_TRUNCATED", "TREE_COMPLETE"
        )) as root:
            with self.assertRaises(MODULE.ValidationError): MODULE.validate(root)

    def test_unbounded_blob_decoder_is_rejected(self) -> None:
        with self.mutated_root(MODULE.FILES[8], lambda value: value.replace(
            "require(blob.size <= maxBytes)", "check(true)"
        )) as root:
            with self.assertRaises(MODULE.ValidationError): MODULE.validate(root)

    def test_missing_complete_snapshot_sha_gate_is_rejected(self) -> None:
        with self.mutated_root(MODULE.FILES[6], lambda value: value.replace(
            "indexedCommitSha == commit.sha", "indexedCommitSha != commit.sha"
        )) as root:
            with self.assertRaises(MODULE.ValidationError): MODULE.validate(root)

    def test_missing_hilt_provider_is_rejected(self) -> None:
        with self.mutated_root(MODULE.FILES[10], lambda value: value.replace(
            "provideRafGitFsGithubApiService", "missingRafGitFsGithubApiService"
        )) as root:
            with self.assertRaises(MODULE.ValidationError): MODULE.validate(root)
        ...
        class TempRoot:
            ...
            def __enter__(self) -> Path:
                for item in MODULE.FILES:
                    source = ROOT / item; target = self.path / item
                    target.parent.mkdir(parents=True, exist_ok=True)
                    text = source.read_text(encoding="utf-8")
                    if item == relative: text = mutate(text)
