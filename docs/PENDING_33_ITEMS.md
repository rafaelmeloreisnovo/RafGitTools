# PENDING 33 ITEMS — Baseline revisado + status pós-refatoração 2026-06-05
...
| P33-04 | 24 | Git commit (amend) | 🆕✅ | `JGitService.commitAmend()` + interface + impl adicionados |
...
| P33-12 | 46 | Syntax highlighting | 🔶 | Detecção de linguagem OK; renderização full highlight depende de UI |
| P33-13 | 47 | Line numbers | 🔶 | Diff/blame têm numeração; exibição inline no FileViewer pendente de UI |
| P33-14 | 48 | File search | 🆕✅ | `JGitService.searchFiles()` — busca por nome e conteúdo na árvore |
| P33-15 | 50 | Breadcrumb navigation | 🔶 | Path de diretórios disponível; componente UI de breadcrumb pendente |
| P33-16 | 51 | File type icons | 🔶 | `detectLanguage()` pronto; mapa visual de ícones pendente de UI |
| P33-17 | 52 | File size display | ✅ | Campo `size` em `GitFile` |
...
| P33-20 | 55 | Branch selector | 🔶 | `getBranches()` OK; componente seletor de referência na UI pendente |
| P33-21 | 56 | Tag selector | 🔶 | `listTags()` OK; seletor dedicado de tag na UI pendente |
| P33-22 | 57 | Repository metadata | 🔶 | Metadados core OK; campos extras (visibility, forks, stars) requerem API |
| P33-23 | 59 | Device authorization flow | ✅ | `OAuthDeviceFlowManager` implementado |
| P33-24 | 61 | Fine-grained PAT support | 🔶 | PAT validado via `/user`; scopes fine-grained não são inspecionados |
| P33-25 | 63 | Token refresh mechanism | 🔶 | `TokenRefreshManager` detecta expiração via headers; OAuth refresh = stub (correto para PATs) |
...
| P33-32 | 70 | Session management | 🔶 | `AuthRepository` persiste estado; expiração de sessão baseada em headers |
...
|---|---|
| ✅ Implementado | 19 |
| 🆕 Implementado nesta execução | 6 |
| 🔶 Parcial | 8 |
| 🔴 Não iniciado | 0 |

**Total P33 com cobertura ≥ funcional: 25/33 (76 %)**
