...
Claim policy: `claim_allowed=false` until the exact capability is closed by source/build/runtime/device evidence appropriate to that capability.
...
| RGT-U0-02 | U0 | exact-head CI | `TOKEN_VAZIO_FINAL_HEAD` | prior BUILD receipts do not automatically prove current `main` | run unit/lint/assemble/security/receipt gates on exact current head | workflow run/job IDs + head SHA + artifact digest + test/lint summary | bind every promotion to exact-head CI |
...
| RGT-U0-05 | U0 | security CI | `PARTIAL` | SARIF permission was repaired, but the TruffleHog raw first cause remained unbound in the observed corrective PR context | reproduce security workflow on exact head and bind first failure cause or PASS | security workflow run, step logs/digests and categorized receipt | do not collapse tooling failure into vulnerability finding or PASS |
...
| RGT-U2-01 | U2 | test coverage | `TOKEN_VAZIO_CURRENT_DENOMINATOR` | historical ~20% coverage is not an exact-head measurement | run pinned coverage tool on exact source set and define denominator | coverage report + source/test file manifest hashes | track branch/line/function separately; avoid single opaque percent |
...
| RGT-U3-01 | U3 | performance/UX | `TOKEN_VAZIO_CONTROLLED_BENCHMARK` | mobile Git/UI/native performance lacks controlled device/repo-size matrix | pinned device, thermal state, repo fixtures and percentile methodology | raw traces + environment + p50/p95/p99 and variance | benchmark after runtime correctness and recovery gates |

## Closure order

`RGT-U0-02 → RGT-U0-05 → RGT-U0-03 → RGT-U0-04 → RGT-U0-01 → RGT-U1-* → RGT-U2-* → RGT-U3-*`

Provider-governance mutation is intentionally after exact-head CI/security observation unless a separate security requirement forces earlier enforcement; any mutation must remain reversible and provider-read-back.

## R3

- `F_ok`: advanced Android/Git source, prior verified devDebug build, dual ABI artifact history, governance dry-run/rollback machinery, active corrective CI work, FNEXT8 validator executed in PR.
- `F_gap`: exact-current-head CI, physical-device chain, release provenance, residual security first-cause binding, real provider/auth/Git fixtures, PTY, external LLaMA runtime, license metadata reconciliation.
- `F_next`: close exact-head and physical evidence before expanding feature count or promoting claims.
