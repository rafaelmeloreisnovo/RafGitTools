...
import com.rafgittools.rafgitfs.sync.RafGitFsOperationRisk
...
class RafGitFsGithubBranchWriter @Inject constructor(
    ...
    override suspend fun execute(
        ...
        if (step.risk != RafGitFsOperationRisk.REMOTE_BRANCH_WRITE) {
            return blocked(step, "REMOTE_RISK_REQUIRED")
        }
        ...
        return when (step.action) {
            RafGitFsPlannedAction.CREATE_BRANCH -> createBranch(plan, step, workspaceId)
            RafGitFsPlannedAction.CREATE_COMMIT -> createCommit(plan, step, workspaceId)
            RafGitFsPlannedAction.PUSH_BRANCH -> pushBranch(plan, step, workspaceId)
            RafGitFsPlannedAction.OPEN_PULL_REQUEST -> openPullRequest(plan, step, workspaceId)
    ...
    suspend fun rollbackToBase(
        ...
        val currentResponse = readApi.resolveCommit(owner, repo, branch)
        val current = currentResponse.body()
        if (!currentResponse.isSuccessful || current == null) {
            return httpFailure(step, "ROLLBACK_BRANCH_READ", currentResponse.code())
        }
        ...
        if (!baseResponse.isSuccessful || base == null) {
            return httpFailure(step, "ROLLBACK_BASE_TREE", baseResponse.code())
        }
        val revertResponse = writeApi.createCommit(
            owner,
            repo,
            RafGitFsCreateCommitRequest(
                message = "revert(rafgitfs): rollback ${plan.requestId.take(8)} to ${baseSha.take(12)}",
                tree = base.tree.sha,
                parents = listOf(current.sha)
            )
        )
        val revert = revertResponse.body()
        if (!revertResponse.isSuccessful || revert == null) {
            return httpFailure(step, "ROLLBACK_COMMIT", revertResponse.code())
        }
        val pushed = writeApi.updateBranchRef(
            owner,
            repo,
            encodeBranch(branch),
            RafGitFsUpdateRefRequest(revert.sha, force = false)
        )
        ...
        upsertMeta(plan, workspaceId, branch, baseSha, revert.sha, "ROLLED_BACK")
    ...
    private suspend fun createBranch(
        ...
        val baseSha = plan.baseCommitSha ?: return blocked(step, "BASE_COMMIT_SHA_REQUIRED")
        val (owner, repo) = splitRepo(plan.repositoryFullName) ?: return blocked(step, "INVALID_REPOSITORY")
        val currentResponse = readApi.resolveCommit(owner, repo, plan.refName)
        val current = currentResponse.body()
        if (!currentResponse.isSuccessful || current == null) {
            return httpFailure(step, "BASE_REF_READ", currentResponse.code())
        }
        if (!current.sha.equals(baseSha, ignoreCase = true)) {
            recordBaseConflict(plan, workspaceId, current.sha)
            workspaceStore.setConflict(workspaceId)
            return blocked(step, "BASE_REF_MOVED:${current.sha.take(12)}")
        }
        val existing = metadata(workspaceId)
        ...
        val response = writeApi.createRef(
            owner,
            repo,
            RafGitFsCreateRefRequest("refs/heads/$branch", baseSha)
        )
        val created = response.body()
        if (!response.isSuccessful || created == null) {
            if (response.code() == 422) {
                val resolvedResponse = readApi.resolveCommit(owner, repo, branch)
                val resolved = resolvedResponse.body()
                if (resolvedResponse.isSuccessful && resolved != null &&
                    resolved.sha.equals(baseSha, ignoreCase = true)
                ) {
                    workspaceDao.upsert(
                        workspace.copy(
                            branchName = branch,
                    ...
                    upsertMeta(plan, workspaceId, branch, baseSha, baseSha, "BRANCH_CREATED")
        ...
        upsertMeta(plan, workspaceId, branch, baseSha, created.`object`.sha, "BRANCH_CREATED")
    ...
    private suspend fun createCommit(
        ...
        if (meta.state in setOf("COMMIT_READY", "PUSHED", "ROLLED_BACK") || meta.state.startsWith("PR_OPEN:")) {
        ...
        val files = try {
            workspaceStore.stagedFiles(workspaceId)
        } catch (error: Exception) {
            return blocked(step, RafGitFsCanonical.sanitize(error.message).orEmpty())
        }
        ...
        val base = baseResponse.body()
        if (!baseResponse.isSuccessful || base == null) {
            return httpFailure(step, "BASE_COMMIT_READ", baseResponse.code())
        }
        ...
        for (file in files.sortedBy { it.path }) {
            ...
            if (!blobResponse.isSuccessful || blob == null) {
                return httpFailure(step, "CREATE_BLOB:${file.path}", blobResponse.code())
            }
        ...
        val treeResponse = writeApi.createTree(
            owner,
            repo,
            RafGitFsCreateTreeRequest(base.tree.sha, treeEntries)
        )
        val tree = treeResponse.body()
        if (!treeResponse.isSuccessful || tree == null) {
            return httpFailure(step, "CREATE_TREE", treeResponse.code())
        }
        ...
        if (!commitResponse.isSuccessful || commit == null) {
            return httpFailure(step, "CREATE_COMMIT", commitResponse.code())
        }
        upsertMeta(plan, workspaceId, branch, baseSha, commit.sha, "COMMIT_READY")
        return observed(step, "COMMIT_READY", commit.sha, branch)
    }

    private suspend fun pushBranch(
        plan: RafGitFsSyncPlan,
        step: RafGitFsPlanStep,
        workspaceId: String
    ): RafGitFsExecutionOutcome {
        val meta = metadata(workspaceId) ?: return blocked(step, "COMMIT_STEP_REQUIRED")
        val branch = meta.path ?: return blocked(step, "BRANCH_METADATA_MISSING")
        ...
        val response = writeApi.updateBranchRef(
            ...
            repo,
            encodeBranch(branch),
            RafGitFsUpdateRefRequest(commitSha, force = false)
        )
        val pushed = response.body()
        if (!response.isSuccessful || pushed == null) {
            return httpFailure(step, "PUSH_BRANCH", response.code())
        }
        upsertMeta(plan, workspaceId, branch, plan.baseCommitSha, pushed.`object`.sha, "PUSHED")
    ...
    private suspend fun openPullRequest(
        ...
        val meta = metadata(workspaceId) ?: return blocked(step, "PUSH_STEP_REQUIRED")
        val branch = meta.path ?: return blocked(step, "BRANCH_METADATA_MISSING")
        if (meta.state.startsWith("PR_OPEN:")) return observed(step, meta.state, meta.localSha, branch)
        ...
        val response = writeApi.openPullRequest(
            ...
            RafGitFsOpenPullRequestRequest(
                title = "RafGitFS: ${workspaceId.take(8)}",
        ...
        if (!response.isSuccessful || pull == null) {
            return httpFailure(step, "OPEN_PULL_REQUEST", response.code())
        }
        workspaceStore.setPublished(workspaceId, branch)
        upsertMeta(plan, workspaceId, branch, plan.baseCommitSha, pull.head.sha, "PR_OPEN:${pull.number}")
        return observed(step, "PR_OPEN:${pull.number}", pull.head.sha, branch)
    }

    private suspend fun recordBaseConflict(
        plan: RafGitFsSyncPlan,
        workspaceId: String,
        currentSha: String
    ) {
    ...
    private suspend fun upsertMeta(
        ...
        stagedDao.upsert(
            StagedOperationEntity(
                operationId = "$workspaceId:publish",
                jobId = null,
                workspaceId = workspaceId,
    ...
    private fun branchName(plan: RafGitFsSyncPlan): String {
        val base = plan.refName.lowercase()
    ...
    private fun httpFailure(step: RafGitFsPlanStep, stage: String, code: Int) =
        RafGitFsExecutionOutcome(
            step,
            "FAILED",
            if (code in 400..499) "TOKEN_VAZIO" else "ERROR",
            null,
    ...
    private fun fallbackStep() = RafGitFsPlanStep(
        ...
        null,
        RafGitFsOperationRisk.REMOTE_BRANCH_WRITE,
        null,
        null,
        true,
        false,
