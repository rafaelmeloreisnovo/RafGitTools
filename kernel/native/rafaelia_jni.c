/**
 * rafaelia_jni.c — RafGitTools JNI bridge for the RAFAELIA engine
 *
 * Adapts _incoming/rafaelia_jni_direct.c to the com.rafgittools package.
 * Zero malloc per call; uses DirectByteBuffer for zero-copy I/O.
 *
 * Native library name: rafaelia
 * Kotlin peer: com.rafgittools.kernel.RafaeliaCore
 */
...
Java_com_rafgittools_kernel_RafaeliaCore_processNative(
        ...
        jobject out_buf)
{
    ...
    /* Input CRC as coherence signal */
    uint32_t c_in = _crc32(in, (size_t)in_len) & 0xFFFFu;

    /* Shannon entropy approximation: unique bytes / 256 * 65535 */
    uint8_t seen[256];
    memset(seen, 0, 256);
    int uniq = 0;
    int scan = in_len < 4096 ? in_len : 4096;
    ...
    if (out_cap >= 16) {
        uint32_t r[4] = { _crc32(in, (size_t)in_len), phi, g_state->phase, g_state->step };
        memcpy(out, r, 16);
        return 16;
    }
...
/* ── stepNative ──────────────────────────────────────────────────────────── */
JNIEXPORT jint JNICALL
Java_com_rafgittools_kernel_RafaeliaCore_stepNative(
        JNIEnv *env, jclass cls,
...
Java_com_rafgittools_kernel_RafaeliaCore_profileNative(
        JNIEnv *env, jclass cls,
        jobject out_buf, jint cap)
{
    ...
    char cpu_online[64] = {0}, freq0[32] = {0}, freq1[32] = {0}, pg[16] = {0};
    int fd; ssize_t n;
    ...
    int pos = 0;
#define W(s) do { const char *_s=(s); while(*_s && pos<cap-1) out[pos++]=*_s++; } while(0)
    W("{\"abi\":\""); W(arch);
    W("\",\"cpus\":\""); W(cpu_online);
    W("\",\"freq0\":\""); W(freq0);
    W("\",\"freq1\":\""); W(freq1);
...
#undef W
    ...
    LOGI("profileNative: %d bytes written", pos);
    return (jlong)pos;
}

/* ── arenaSizeNative ─────────────────────────────────────────────────────── */
JNIEXPORT jint JNICALL
Java_com_rafgittools_kernel_RafaeliaCore_arenaSizeNative(
        JNIEnv *env, jclass cls)
{
    (void)env; (void)cls;
...
/* ── crc32Native ─────────────────────────────────────────────────────────── */
JNIEXPORT jint JNICALL
Java_com_rafgittools_kernel_RafaeliaCore_crc32Native(
        JNIEnv *env, jclass cls,
