...
if grep -En '(^|[^[:alnum:]_])(for|while)[[:space:]]*\(|(^|[^[:alnum:]_])do[[:space:]]*\{' \
    src/*.c ../rafcode_route_v1/src/*.c >/dev/null; then
    printf '%s\n' 'FAIL: source loop present in runtime C' >&2
    exit 1
fi

if grep -En '(^|[^[:alnum:]_])(malloc|calloc|realloc|free|memcpy|memmove|memset)[[:space:]]*\(' \
    src/*.c ../rafcode_route_v1/src/*.c >/dev/null; then
